...
(function buildSidebar() {
  ...
  const CHAPTERS = [
    { n:'01', label:'Introduction',        icon:'rocket_launch',   color:G.foundations, group:'Foundations' },
    { n:'02', label:'Context Engineering', icon:'psychology',      color:G.foundations, group:'Foundations' },
    ...
    { n:'05', label:'MCP',                 icon:'hub',             color:G.cli,         group:'CLI & Tools'  },
    { n:'06', label:'GitHub Integration',  icon:'merge_type',      color:G.cli,         group:'CLI & Tools'  },
    { n:'07', label:'Advanced Workflows',  icon:'account_tree',    color:G.advanced,    group:'Advanced'     },
    { n:'08', label:'Subagents',           icon:'smart_toy',       color:G.advanced,    group:'Advanced'     },
    { n:'09', label:'Output Styles',       icon:'format_shapes',   color:G.advanced,    group:'Advanced'     },
    { n:'10', label:'Agent Skills',        icon:'extension',       color:G.expert,      group:'Expert'       },
    { n:'11', label:'Desktop',             icon:'desktop_windows', color:G.expert,      group:'Expert'       },
    { n:'12', label:'Deep Agents',         icon:'model_training',  color:G.expert,      group:'Expert'       },
  ...
  let html = `
    ...
    <a class="sb-home${!seg ? ' active' : ''}" href="index.html">
      <span class="sb-home-icon"><span class="material-symbols-rounded">home</span></span>
      <span>Overview</span>
    </a>
  ...
  Object.entries(groups).forEach(([grp, chs]) => {
    ...
    chs.forEach(ch => {
      ...
      <a class="sb-link${on ? ' active' : ''}" href="chapter-${ch.n}.html" style="--ch-color:${ch.color}">
        <span class="sb-ch-icon"><span class="material-symbols-rounded">${ch.icon}</span></span>
...
(function initIllustrations() {
  ...
  const CHAPTERS = {
    'chapter-01': {
      header: YM('02'),
      sects: [[1, PT(pt[0]), 'r'], [3, FP(fp[0]), 'l'], [5, PH(ph[0]), 'r']],
    },
    'chapter-02': {
      header: YM('03'),
      sects: [[1, PT(pt[1]), 'r'], [3, PH(ph[1]), 'l'], [5, FP(fp[1]), 'r']],
    },
    'chapter-03': {
      header: YM('04'),
  ...
  const cfg  = CHAPTERS[page];
  if (!cfg) return;

  /* 1. Page-header illustration (fixed position, right side) */
  const header = document.querySelector('.page-header');
  if (header && cfg.header) {
    const img = document.createElement('img');
    img.src = cfg.header;
    img.className = 'page-illus';
    img.alt = ''; img.loading = 'lazy'; img.decoding = 'async';
    header.appendChild(img);
  }

  /* 2. Section float illustrations */
  if (cfg.sects && cfg.sects.length) {
    const sections = document.querySelectorAll('.main-content .section');
    cfg.sects.forEach(([idx, url, side]) => {
      ...
      /* insert after the first .lead or after h2 */
      const anchor = sect.querySelector('.lead') || sect.querySelector('h2');
      if (anchor && anchor.nextSibling) {
        sect.insertBefore(img, anchor.nextSibling);
      } else {
